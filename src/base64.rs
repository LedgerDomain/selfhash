...
pub fn base64_decode_256_bits<'a>(
    ...
    require!(
        input_str.len() == 43,
        "expected 43 base64 chars but got {}",
        input_str.len()
    );
    use base64::Engine;
    base64::engine::general_purpose::URL_SAFE_NO_PAD
    ...
    require!(buffer[32] == 0, "does not parse as 256 bit value");
...
pub fn base64_decode_512_bits<'a>(
    ...
    require!(
        input_str.len() == 86,
        "expected 86 base64 chars but got {}",
        input_str.len()
    );
    use base64::Engine;
    base64::engine::general_purpose::URL_SAFE_NO_PAD
        .decode_slice(input_str.as_bytes(), buffer)
        .map_err(|e| error!("base64 decode of 512 bit value failed: {}", e))?;
    // Ensure that the last bytes are zero, otherwise there were more than 512 bits in the base64 string.
    require!(
        buffer[64] == 0 && buffer[65] == 0,
        "does not parse as 512 bit value"
    );
    // Cut off the last byte, which we know is zero.
